...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($title) && empty($description) && empty($description2) && !$mainImageName && !$mainImage2Name) {
        ...
        $stmt = $conn->prepare("INSERT INTO explore_highbrows (main_image, main_image2, title, description, description2) VALUES (?, ?, ?, ?, ?)");
        $stmt->bind_param("sssss", $mainImageName, $mainImage2Name, $title, $description, $description2);
        if ($stmt->execute()) {
            header("Location: show_explore.php?success_message=" . urlencode("Explore content added successfully."));
            exit();
        } else {
    ...
    <div class="form-card">
        <form method="POST" enctype="multipart/form-data">
            ...
            <div class="form-footer mt-4 text-end">
                ...
                <a href="show_explore.php" class="btn btn-secondary ms-2">Back to List</a>
            </div>
        </form>
    </div>
</div>
