...
$id = $_GET['id'] ?? null;
if (!$id) {
    header('Location: show_explore.php');
    exit();
}

$query = "SELECT * FROM explore_highbrows WHERE id=?";
...
if (isset($_POST['update'])) {
    ...
    $stmt = $conn->prepare("UPDATE explore_highbrows SET main_image=?, main_image2=?, title=?, description=?, description2=? WHERE id=?");
    $stmt->bind_param("sssssi", $main_image, $main_image2, $title, $description, $description2, $id);
    if ($stmt->execute()) {
        header("Location: show_explore.php?message=" . urlencode("Record updated successfully."));
        exit();
    } else {
    ...
    <div class="form-card">
        <form method="POST" enctype="multipart/form-data">
            ...
            <div class="form-footer mt-4 text-end">
                ...
                <a href="show_explore.php" class="btn btn-secondary ms-2">Cancel</a>
            </div>
        </form>
    </div>
</div>
