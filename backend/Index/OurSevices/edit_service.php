...
if (!isset($conn) || $conn->connect_error) {
    error_log("Database connection failed in edit_service.php: " . ($conn->connect_error ?? 'Connection object not found'));
...
if ($id === 0) {
    header('Location: show_services.php?error=Invalid+service+ID+provided.');
    exit();
}

// Fetch existing data for the service
$stmt_fetch = $conn->prepare("SELECT * FROM our_services WHERE id = ?");
if (!$stmt_fetch) {
...
if (!$row) {
    ...
    header('Location: show_services.php?error=Service+not+found.');
    exit();
}
...
if (isset($_POST['update'])) {
    ...
    if (empty($message)) {
        $stmt_update = $conn->prepare("UPDATE our_services SET main_image=?, title=?, description=? WHERE id=?");
        if (!$stmt_update) {
            ...
            if ($stmt_update->execute()) {
                header('Location: show_services.php?success=Service+updated+successfully!');
                exit();
            } else {
...
<div class="main container py-4">
    <header class="top-header mb-4 enhanced-shadow">
        <div class="d-flex flex-column flex-md-row justify-content-between align-items-start align-items-md-center gap-3">
            <div class="d-flex align-items-start">
                ...
                <div>
                    <h1 class="h4 fw-bold text-dark mb-1">Edit Service</h1>
                    <small class="text-muted">Update details of your service.</small>
                </div>
            </div>
            <nav aria-label="breadcrumb">
                <ol class="breadcrumb bg-transparent mb-0">
                    ...
                    <li class="breadcrumb-item active text-dark fw-semibold" aria-current="page">Edit Service</li>
                </ol>
            </nav>
        </div>
    </header>
    ...
    <div class="form-header text-start">
        <h2 class="form-heading">Update Service Details</h2>
        <p class="form-subheading">Modify the information below to update this service.</p>
    </div>
            ...
            <div class="row d-flex flex-column gap-3">
                <div class="col-sm-12">
                    <div class="form-group mb-3">
                        <label for="main_image" class="form-label">Current Image</label><br>
                        <?php if (!empty($row['main_image'])) : ?>
                            <div class="mb-2">
                                <img src="uploads/services/<?= htmlspecialchars($row['main_image']) ?>" class="img-preview rounded" alt="Current Service Image">
                            </div>
                        <?php else : ?>
                            <p class="text-muted">No image uploaded for this service.</p>
                        <?php endif; ?>
                        <input type="file" name="main_image" id="main_image" class="form-control" accept="image/*" />
                        <small class="form-text text-muted">Upload a new image (JPG, JPEG, PNG, GIF). Max size: 5MB. Leave blank to keep current image.</small>
