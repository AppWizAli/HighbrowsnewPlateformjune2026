...
@endif 
                ...
                <div class="card shadow-lg border-0">
                    ...
                    <div class="card-body bg-light">
                        <form action="{{ route('college.update', $college->id) }}" method="POST">
                            @csrf
                            @method('PUT') <!-- Specifies the HTTP method to be PUT for updates -->
                            
                            <div class="row">
                                <div class="col-md-6">
                                    <div class="form-group mb-3">
                                        <label for="college_name" class="form-label">College Name</label><span style="color: red"> *</span>
                                        <input type="text" class="form-control {{ $errors->has('college_name') ? 'is-invalid' : '' }}" id="college_name" name="college_name" value="{{ old('college_name', $college->college_name) }}">
                                        @error('college_name')
    ...
    <div class="form-group mb-3">
        ...
        <input type="number" class="form-control {{ $errors->has('college_fee') ? 'is-invalid' : '' }}" id="college_fee" name="college_fee" value="{{ old('college_fee', $college->college_fee) }}" step="0.01" min="0">
        @error('college_fee')
            <div class="invalid-feedback">{{ $message }}</div>
        @enderror
    </div>
